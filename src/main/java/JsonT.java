...
//DEPS mobi.mofokom:graalson-trax:1.0.3
//DEPS mobi.mofokom:graalson:1.0.6
//FILES README.md
//MAIN_CLASS JsonT
...
public class JsonT {
    ...
    public static void main(String[] args)
        ...
        if (
            args.length == 0 ||
            Arrays.asList("/?", "-?", "-h", "--help").contains(args[0].trim())
        ) {
            try (
        ...
        String templateFile =
            args.length < 1 ? IDENTITY : args[0].isBlank() ? IDENTITY : args[0];
        String inputJson =
            args.length < 2 ? STDIN : args[1].isBlank() ? STDIN : args[1];
        String outputFile =
            args.length < 3 ? STDOUT : args[2].isBlank() ? STDOUT : args[2];
        String charset =
            args.length < 4 ? UTF8 : args[3].isBlank() ? UTF8 : args[3];
        String spaces =
            args.length < 5 ? SPACES : args[4].isBlank() ? SPACES : args[4];
