...
public class JsonT {
    ...
    public static final String STDIN = "-", STDOUT = "-", UTF8 = "UTF-8", IDENTITY = "identity.js";
    ...
    public static void main(String[] args) throws IOException, TransformerConfigurationException, TransformerException {
        if (args.length == 0) {
            try (PrintWriter out = new PrintWriter(System.err); InputStream in = ClassLoader.getSystemClassLoader().getResourceAsStream("help.md")) {
                int c;
                while ((c = in.read()) != -1) {
                    out.write(c);
                }
        ...
        JsonWriter jwriter = wfactory.createWriter(STDOUT.equals(outputFile) ? new PrintWriter(System.out) : new FileWriter(outputFile));
        Reader reader = null;
        JsonReader jreader = null;

        System.err.println("template: " + templateFile + " source:" + inputJson + " result:" + outputFile + " charset:" + charset);
        if (STDIN.equals(inputJson)) {
            ReadableByteChannel channel = Channels.newChannel(System.in);

            reader = new InputStreamReader(Channels.newInputStream(channel),
                    (UTF8.equalsIgnoreCase(charset)) ? java.nio.charset.StandardCharsets.UTF_8 : java.nio.charset.Charset.forName(charset));
            jreader = Json.createReader(reader);
        } else if (System.console() != null && (inputJson == null || inputJson.equals("-"))) {
            reader = new InputStreamReader(System.in,
                    (UTF8.equalsIgnoreCase(charset)) ? java.nio.charset.StandardCharsets.UTF_8 : java.nio.charset.Charset.forName(charset));
            jreader = Json.createReader(reader);
        } else if (inputJson != null) {
            try {
                InputStream jsonStream = ClassLoader.getSystemClassLoader().getResourceAsStream(inputJson);
                if (jsonStream == null) {
                    jsonStream = new java.io.FileInputStream(inputJson);
                }
                reader = new InputStreamReader(jsonStream,
                        (charset == null || charset.equalsIgnoreCase("unknown")) ? java.nio.charset.StandardCharsets.UTF_8 : java.nio.charset.Charset.forName(charset));
                jreader = Json.createReader(reader);
        ...
        System.err.println("template: " + template + " source:" + source + " result: " +result + " writer:" + jwriter);
